...
/* Credits to https://github.com/ArpitShukIa/AdventOfCode2022/blob/main/src/Day17.kt */

typealias Rock = Set<Pair<Int, Int>>

fun main() {

    dropRocks("src/main/kotlin/day17/test.txt")
    dropRocks("src/main/kotlin/day17/input.txt")
}
fun dropRocks(fileName: String) {
    val jetDirections = readStringFile(fileName)[0]
    ...
    while (rocksDropped < totalRocksToDrop) {
        ...
        while (true) {
            ...
            jetIndex = (jetIndex + 1) % jetDirections.length
            rock = rock.moveDown()
            if (rock.intersect(grid).isNotEmpty()) {
                rock = rock.moveUp()
                grid += rock
                ...
                val key = Triple(jetIndex, rocksDropped % 5, topGrid)
